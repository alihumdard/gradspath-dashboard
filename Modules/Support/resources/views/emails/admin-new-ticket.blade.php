        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:640px; background:#ffffff; border-radius:22px; overflow:hidden; box-shadow:0 18px 50px rgba(17,24,39,.08);">
          <tr>
            <td style="padding:28px 32px; background:linear-gradient(135deg,#6f4cf6,#23293a); color:#ffffff;">
              ...
              <h1 style="margin:12px 0 8px; font-size:28px; line-height:1.2;">New support ticket</h1>
              <p style="margin:0; font-size:15px; line-height:1.6;">{{ $ticket->ticket_ref }} from {{ $ticket->user?->name ?? 'Unknown user' }}</p>
            </td>
          </tr>
          <tr>
            <td style="padding:28px 32px;">
              <p style="margin:0 0 10px; font-size:15px;"><strong>Subject:</strong> {{ $ticket->subject }}</p>
              <p style="margin:0 0 10px; font-size:15px;"><strong>User:</strong> {{ $ticket->user?->email ?? '-' }}</p>
              <p style="margin:16px 0; font-size:14px; line-height:1.7; color:#4b5563;">{{ $ticket->message }}</p>
              <a href="{{ $adminUrl }}" style="display:inline-block; padding:13px 18px; border-radius:12px; background:#6f4cf6; color:#ffffff; text-decoration:none; font-weight:700;">Open ticket</a>
            </td>
          </tr>
        </table>
      </td>
